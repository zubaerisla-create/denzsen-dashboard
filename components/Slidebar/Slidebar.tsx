...
const sidebarItems = [
  ...
  { name: 'Cases', href: '/dashboard/case-list', icon: FileText },
  { name: 'User Management', href: '/dashboard/users', icon: Users },
  { name: 'Analyst Management', href: '/dashboard/analyst', icon: BarChart3 },
  { name: 'Contact Us', href: '/dashboard/contact', icon: Mail },
  { name: 'CPIN Portal', href: '/dashboard/cop-portal', icon: FileText },
...
const Slidebar = ({
  ...
  const filteredItems = sidebarItems.filter(item => {
    if (!userRole) return true; // Show all while loading or if not found
    
    const role = userRole.toLowerCase();
    
    // Admin, Director have full access
    if (role === 'admin' || role === 'director') return true;
    
    // Analyst access
    if (role === 'analyst') {
      return ['Dashboard Overview', 'Cases', 'CPIN Portal', 'Settings'].includes(item.name);
    }
    
    // Moderator access
    if (role === 'moderator') {
      return ['Dashboard Overview', 'Contact Us', 'CPIN Portal', 'Settings'].includes(item.name);
    }
    
    // LEO, PI access
    if (role === 'leo' || role === 'pi') {
      return ['Dashboard Overview', 'CPIN Portal', 'Settings'].includes(item.name);
    }
    
    return ['Dashboard Overview', 'Settings'].includes(item.name);
  });
      ...
      <div className="flex items-center justify-between h-16 px-6 bg-white border-b border-gray-200">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex flex-col">
            <h2 className="font-bold text-lg">CPIN</h2>
      ...
      <nav className="mt-4 flex flex-col h-[calc(100vh-8rem)]">
        ...
        <div className="border-t border-gray-200">
          <button
            ...
            className="w-full flex items-center px-6 py-3 text-black hover:bg-gray-100 hover:text-[#507493] transition-colors"
          >
